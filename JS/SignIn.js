...
function onSignIn(){
    try {
        ...
        else
        {
            ...
            if(userLst.length>0) 
            {
                var user =  userLst.find(userObj =>  userObj.email ===email && userObj.pwd ===password);
                if(user==undefined){                            
                    throw {toString: function() { return "user doesn't exist or credentials doesn't matching"; } };
                }
                else{
